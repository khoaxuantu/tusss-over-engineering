...
import { Store, StoreInsert } from "../../models/store.model";
import { StoreWriteRepository } from "../store.repository";

describe(StoreWriteRepository.name, () => {
  const dbClient = new MockDbClient();

  let repository: StoreWriteRepository;

  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [StoreWriteRepository, { provide: DbClientProvider, useValue: dbClient }],
    }).compile();

    repository = moduleRef.get(StoreWriteRepository);
  });
  ...
  describe("insertOne", () => {
    it("should insert", async () => {
      ...
      await repository.insertOne(new StoreInsert());
  ...
  describe("insertMany", () => {
    it("should insert", async () => {
      dbClient.mockInsertQueryOnce({
        execute: jest.fn().mockResolvedValue([new Store()]),
      });
      const spyInsert = dbClient.insertInto.mockClear();
      await repository.insertMany([new StoreInsert()]);
      expect(spyInsert).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe("updateAndReturn", () => {
    ...
    it("should return defined if record found", async () => {
      dbClient.mockUpdateQueryOnce({
        executeTakeFirst: jest.fn().mockResolvedValue(StoreInsert.create()),
      });
      const res = await repository.updateAndReturn(1, repository.updater);
