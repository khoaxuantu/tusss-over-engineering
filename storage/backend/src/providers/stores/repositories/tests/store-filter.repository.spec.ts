...
import { StoreFilterRepository } from "../store.repository";

describe(StoreFilterRepository.name, () => {
  const db = new MockDbClient();
  const pagination = new MockPaginationHelper();

  let repository: StoreFilterRepository;
  ...
  beforeAll(async () => {
    ...
    repository = moduleRef.get(StoreFilterRepository);
  });
